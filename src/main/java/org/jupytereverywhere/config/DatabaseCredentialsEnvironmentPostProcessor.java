  ...
  public void postProcessEnvironment(
    ...
    if (hasCredentialsJson && (hasUsername || hasPassword)) {
      throw new IllegalStateException(
          "Only one of DB_CREDENTIALS or (DB_USERNAME and DB_PASSWORD) may be set, not both.");
    }
    ...
    if (hasCredentialsJson) {
      try {
        ...
        System.err.println(
            "[DatabaseCredentialsEnvironmentPostProcessor] Failed to parse DB_CREDENTIALS: "
                + e.getMessage());
        throw new IllegalStateException(
            "DB_CREDENTIALS is set but could not be parsed as valid JSON with 'username' and 'password' fields.",
            e);
      }
    } else if (!hasUsername || !hasPassword) {
      throw new IllegalStateException(
          "Either DB_CREDENTIALS or both DB_USERNAME and DB_PASSWORD environment variables must be set.");
    } else {
